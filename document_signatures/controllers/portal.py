...
class ExtendCustomerPortal(CustomerPortal):
    ...
    @http.route(
        ["/my/dms/file/<int:document_id>/<int:directory_id>/sign_complete"],
        type="http",
        auth="public",
        methods=["POST"],
        csrf=False,
    ...
    def complete_signing(self, document_id, directory_id, access_token, **res):
        ...
        res = api_signport.sudo().document_signport_post(data, document_id, "/CompleteSigning")
        res_json = json.dumps(res)



        return self.portal_my_dms_directory(dms_directory_id=directory_id, access_token=None)
    ...
    def start_sign(self, document_id):
        ...
        res = api_signport.sudo().post_sign_document(
            ...
            directory_id=data.get("params", {}).get("directory"),
